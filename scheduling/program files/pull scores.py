...
def seed_pool(pool):
    ...
    for team in pool:
        if [t[0] for t in pool].count(team[0]) > 1 and len(team) > 3:
            if disp: print('Exact same point total and differential... need to address\n',pool)
    return pool
...
def print_playoffs_1(e,rnd,rnd2,d,p):
    global alpha_list
    global key
    global playoff_tree
    global scores
    ...
    if have_scores:
        for game in playoff_tree[e][0][rnd]:
            if type(game[0]) == int:
                print('Game ' + alpha_list[playoff_tree[e][0][rnd].index(game)] + ':\t\t' + div[p][game[0]] + ' v ' + div[p][game[1]])
            else:
                print('Game ' + alpha_list[playoff_tree[e][0][rnd].index(game)] + ':\t\t' + div[p+int(game[0])][int((game[0] - int(game[0])) * 10)] + ' v ' + div[p+int(game[1])][int((game[1] - int(game[1])) * 10)])
    else:
        for game in playoff_tree[e][0][rnd]:
            print(game)
            if type(game[0]) == int:
                print('Game ' + alpha_list[playoff_tree[e][0][rnd].index(game)] + ':\t\tPool ' + str(p+1) + ' (' + place_key[str(game[0])] + ' Place) v  Pool ' + str(p+1) + ' (' + place_key[str(game[1])] + ' Place)')
            else:
                print('Game ' + alpha_list[playoff_tree[e][0][rnd].index(game)] + ':\t\tPool ' + str(p+int(game[0])+1) + ' (' + place_key[str(abs(int(round((game[0] - int(game[0])) * 10))))] + ' Place) v  Pool ' + str(p+int(game[1])+1) + ' (' + place_key[str(abs(int(round((game[1] - int(game[1])) * 10))))] + ' Place)')
...
pAllOpp = [[[[],[]] for pool in div] for div in listTeamsDiv]
...
try:
    have_scores = True
    with open('program files/patriot-13~.csv', newline='') as f:
        reader = csv.reader(f)
        for row in reader:
...
for d in range(len(scores)):
    ...
    for p in range(len(divInfo[0])):
        ...
        print_playoffs_1(sz, rnd_default, rnd2_default, d, p)
