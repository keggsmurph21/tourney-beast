...
lTeamsDiv = [[],listTeamsDiv]
lDivInfo = [[],listDivInfo]
lComp = [[],listComp]
pBlocks = int(gamesPerDay/pNumberOfGames)

have_scores_rd2 = False
playoff_rd2 = ['', '', '', '',
               [['A (Winner)', 'B (Winner)'], ['A (Loser)', 'B (Loser)']], '',
               [['A (Winner)', 'B (Winner)'], ['A (Loser)', 'C (Winner)'], ['B (Loser)', 'C (Loser)']], '',
               [['A (Winner)', 'B (Winner)'], ['A (Loser)', 'B (Loser)'], ['C (Winner)', 'D (Winner)'], ['C (Loser)', 'D (Loser)']], '',
               [['A (Winner)', 'B (Winner)'], ['A (Loser)', 'B (Loser)'], ['C (Winner)', 'D (Winner)'], ['C (Loser)', 'E (Winner)'], ['D (Loser)', 'E (Loser)']], '',
               [['A (Winner)', 'B (Winner)'], ['A (Loser)', 'B (Loser)'], ['C (Winner)', 'D (Winner)'], ['C (Loser)', 'D (Loser)'], ['E (Winner)', 'F (Winner)'], ['E (Loser)', 'F (Loser)']], '',
               [['A (Winner)', 'B (Winner)'], ['A (Loser)', 'B (Loser)'], ['C (Winner)', 'D (Winner)'], ['C (Loser)', 'D (Loser)'], ['E (Winner)', 'F (Winner)'], ['E (Loser)', 'G (Winner)'], ['F (Loser)', 'G (Loser)']]]
...
for dayindex in range(1,len(lTeamsDiv)):    
    ...
    for divID in range(len(pAllOpp)):
        for poolID in range(len(pAllOpp[divID])):
            if len(pAllOpp[divID][poolID][0]):
                size = len(pAllOpp[divID][poolID][0])
                listPieces.append([size,divID,poolID,0])
    ...
    fGames = [[[[] for timeslot in range(numberOfGames)] for pool in div] for div in pAllOpp]

    for div in range(len(pAllOpp)):
        for pool in range(len(pAllOpp[div])):
            pMatchups = pAllOpp[div][pool][0]
            for pMatchup in pMatchups:
                ...
                for opp in pMatchup:
                    ...
                    team = opp[1]
                    if have_scores:
                        matchup.append(listDivInfo[div] + ': ' + listTeamsDiv[div][adjPool][team])
                    else:
                        matchup.append(listDivInfo[div] + ': ' + str(div + 1) + '-' + str(adjPool + 1) + '-' + alpha_list[pMatchups.index(pMatchup)] + ' (' + place_key[str(team)] + ' Place)')
                    if have_scores_rd2:
                        pass
                    else:
                        matchup_rd2.append(listDivInfo[div] + ': ' + str(div + 1) + '-' + str(adjPool + 1) + '-' + playoff_rd2[2 * len(pMatchups)][pMatchups.index(pMatchup)][pMatchup.index(opp)])
                fGames[div][pool][0].append(matchup)
                fGames[div][pool][1].append(matchup_rd2)
